...
class SibylServerUdpTextProtocol(DatagramProtocol):
    ...
    def datagramReceived(self, datagram, host_port):
        ...
        """
        reception=datagram.decode("utf-8")
        print(reception)
        envoie= self.sibylServerProxy.generateResponse(reception[9:])
        print(reception[:12]+envoie)
        #print(host_port[0],host_port[1])
        self.transport.connect(host_port[0],host_port[1])
        self.transport.write((reception[:12]+envoie).encode('utf-8'))
